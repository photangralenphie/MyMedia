...
class MediaImporter {
	...
	private static func readMovieMetadata(file: MP42File, moc: ModelContext) throws {
		let movie = try createMovieFromFile(file: file)
		moc.insert(movie)
	}
	...
	public static func createEpisodeFromFile(file: MP42File) throws -> Episode {
		let artwork = file.tryGetImageMetaDataValue(for: MP42MetadataKeyCoverArt, artworkType: .episodeImage)
...
extension MP42File {
	...
	// Artwork
	func tryGetImageMetaDataValue(for key: String, artworkType: ArtworkType) -> Data? {
		let artworks = self.metadata.metadataItemsFiltered(byIdentifier: key)
		if artworks.isEmpty { return nil }
		
		if artworks.endIndex > artworkType.index {
			return artworks[artworkType.index].imageValue?.data
		}
		
		return artworks.first?.imageValue?.data
	}
}
